...
static void printIsolatedSensorDebug(unsigned long now) {
    ...
    Serial.print(currentSensors.heartBPM);
    Serial.print(" det=");
    Serial.print(currentSensors.pulseDetected ? "Y" : "N");
    Serial.print(" abn=");
    Serial.print(currentSensors.heartAbnormal ? "Y" : "N");
    Serial.print(" | faults M/U/I/H=");
    ...
    Serial.print("/");
    Serial.println(systemFaults.heart_fail);
...
void loop() {
    ...
    if (currentSituation == FALL_DETECTED || systemFaults.imu_fail) {
        ...
        if (currentMode != HIGH_STRESS && currentMode != EMERGENCY) {
            ...
            if (DEBUG_MODE) Serial.println("HIGH_STRESS MODE: Close obstacle + abnormal heart rate (isolated)");
        }
    } else if (currentMode == HIGH_STRESS && currentSituation != HIGH_STRESS_EVENT) {
        setMode(NORMAL);
        currentEmergencyType = EMERGENCY_NONE;
        if (DEBUG_MODE) Serial.println("HIGH_STRESS CLEARED: returning to isolated normal mode");
