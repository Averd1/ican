...
async def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default=str(Path(__file__).resolve().parent / "test_results"),
        help="Directory where CSV output should be saved (default: test_results next to script)",
    )
    ...
    filename = "smartcane_ble_v2_" + datetime.datetime.now().strftime("%H%M%S_%Y%m%d") + ".csv"
